...
test('given a valid schedule, when schedule is created, then schedule is created with those values', () => {
    // given
    courseDb.getCourseById = mockCourseDbGetCourseById.mockReturnValue(course);

    lecturerDb.getLecturerById = mockLecturerDbGetLecturerById.mockReturnValue(lecturer);
...
test('given a schedule with an existing course and lecturer, when schedule is created, then an error is thrown', () => {
    ...
    // then
    expect(createSchedule).toThrow('This course is already scheduled for this lecturer.');
});
