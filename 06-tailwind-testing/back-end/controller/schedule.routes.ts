...
import { EnrollmentInput, ScheduleInput } from '../types';
 ...
 */
scheduleRouter.get('/', async (req: Request & { auth: any }, res: Response) => {
    try {
        const { username, role } = req.auth;
        const schedules = await scheduleService.getSchedule({ username, role: role.name });
